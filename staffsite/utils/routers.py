...
class EmailAddressRouter(object):
    """
    A router to control all database operations on models in the
    member application.
    """
    def db_for_read(self, model, **hints):
        ...
        Attempts to read member models go to website.
        """
        if model._meta.app_label == 'simple_email_confirmation':
            return 'website'
        return None
    def db_for_write(self, model, **hints):
        """
    ...
    def allow_relation(self, obj1, obj2, **hints):
        ...
        Allow relations if a model in the member app is involved.
        """
        if (obj1._meta.app_label == 'simple_email_confirmation' or
            obj2._meta.app_label == 'simple_email_confirmation'):
                return True
        return None
    ...
    def allow_migrate(self, db, model):
        ...
        """
        if db == 'website':
             return model._meta.app_label == 'simple_email_confirmation'
        elif model._meta.app_label == 'simple_email_confirmation':
             return False
        return None
